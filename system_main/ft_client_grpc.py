...
#                                          display_name="")
#         try:
#             r = self.auth_stub.CreateUser(req)
...
#         try:
#             r = self.auction_stub.StartAuction(req)
...
#                                         amount=amt)
#         try:
#             r = self.auction_stub.SubmitBid(req)
...
#         try:
#             r = self.auction_stub.GetWinner(req)
#             if r.status == "success":
...
class FTClient:
    ...
    def safe_rpc(self, rpc_call, request):
        """
        Wrap an RPC call with retry on UNAVAILABLE, reconnecting as needed.
        """
        last_exc = None
        for attempt in range(self.max_retries):
            try:
                ...
                if e.code() == grpc.StatusCode.UNAVAILABLE:
                    ...
                    break
        # if we exit loop, raise or log error
        raise last_exc
